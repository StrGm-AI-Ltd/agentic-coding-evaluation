...
public final class Trajectory {
    ...
    public static List<Turn> turnsFromProxy(List<Map<String, Object>> recs) {
        List<Turn> turns = new ArrayList<>();
        for (Map<String, Object> r : recs) {
            if (!String.valueOf(r.getOrDefault("path", "")).startsWith("/v1/chat/completions")) continue;
            Map<String, Object> resp0 = r.get("response") instanceof Map<?, ?> m ? (Map<String, Object>) m : Map.of();
            Map<String, Object> resp = resp0;
            List<ToolCall> tcs = new ArrayList<>();
            if (resp0.containsKey("choices")) {   // non-streamed shape -> normalise
                Map<String, Object> ch = ((List<Map<String, Object>>) resp0.getOrDefault("choices", List.of())).get(0);
                Map<String, Object> msg = ch.get("message") instanceof Map<?, ?> mm ? (Map<String, Object>) mm : Map.of();
                for (Map<String, Object> t : (List<Map<String, Object>>) msg.getOrDefault("tool_calls", List.of())) {
                    Map<String, Object> fn = t.get("function") instanceof Map<?, ?> f ? (Map<String, Object>) f : Map.of();
            ...
            Map<String, Object> u = resp.get("usage") instanceof Map<?, ?> uu ? (Map<String, Object>) uu : Map.of();
            ...
            Map<String, Object> pd = u.get("prompt_tokens_details") instanceof Map<?, ?> p ? (Map<String, Object>) p : Map.of();
            String reasoning = String.valueOf(resp.getOrDefault("reasoning", ""));
            String content = String.valueOf(resp.getOrDefault("content", ""));
    ...
    static boolean isTestOrBuild(ToolCall c) {
    ...
    static String argPath(String args) {
        try {
            JsonNode a = JSON.readTree(args);
            String p = a.path("path").asText(a.path("file_path").asText(a.path("filePath").asText("")));
    ...
    static boolean inWindows(Turn t, List<Map<String, Object>> windows) {
        if (windows == null || windows.isEmpty() || t.ts() == null) return false;
        try {
            OffsetDateTime ts = OffsetDateTime.parse(t.ts().replace("Z", "+00:00"));
    ...
    public static Map<String, Object> analyze(List<Turn> turns, Map<String, Object> derived, List<Map<String, Object>> dockerWindows) {
        Map<String, Object> d = derived == null ? Map.of() : derived;
        int ceiling = d.get("usable_context") instanceof Number n ? n.intValue() : 65536;
        int trigger = d.get("compaction_trigger_tokens") instanceof Number n ? n.intValue() : 28000;
        double floor = d.get("min_decode_tps") instanceof Number n ? n.doubleValue() : 10;
        ...
        List<Integer> pt = turns.stream().filter(t -> t.promptTokens() != null).map(t -> ((Number) t.promptTokens()).intValue()).toList();
        List<Integer> ct = turns.stream().filter(t -> t.completionTokens() != null).map(t -> ((Number) t.completionTokens()).intValue()).toList();
        ...
        List<Double> lat = turns.stream().filter(t -> t.latency() instanceof Number).map(t -> ((Number) t.latency()).doubleValue()).toList();
        ...
        List<Double> ttft = turns.stream().filter(t -> t.ttft() instanceof Number).map(t -> ((Number) t.ttft()).doubleValue()).toList();
        if (!ttft.isEmpty()) s.put("ttft_mean_sec", Math.round(ttft.stream().mapToDouble(Double::doubleValue).average().orElse(0) * 100) / 100.0);
        List<Double> g = turns.stream().filter(t -> t.genTps() instanceof Number).map(t -> ((Number) t.genTps()).doubleValue()).toList();
        ...
        List<Turn> out = turns.stream().filter(t -> !t.inDockerWindow()).toList(), inw = turns.stream().filter(Turn::inDockerWindow).toList();
        Map<String, Object> byCtx = new LinkedHashMap<>();
        ...
        int rc = turns.stream().mapToInt(Turn::reasoningChars).sum(), cc = turns.stream().mapToInt(Turn::contentChars).sum();
        s.put("reasoning_share_pct", rc + cc > 0 ? Math.round(1000.0 * rc / (rc + cc)) / 10.0 : null);
        Map<String, Integer> finishes = new LinkedHashMap<>();
        ...
        List<Object[]> seqf = calls.stream().filter(c -> List.of("edit", "write", "read").contains(c[1])).toList();
        int pp = 0;
    ...
    static Object medGenTps(List<Turn> turns, java.util.function.Predicate<Turn> regime) {
    ...
    public static Map<Object, List<String>> toolResults(List<Map<String, Object>> recs) {
        Map<Object, List<String>> out = new LinkedHashMap<>();
        Map<String, Object> prevSeq = new LinkedHashMap<>();   // per task tag: parallel tasks interleave in the merged journal
        for (Map<String, Object> r : recs) {
            if (!String.valueOf(r.getOrDefault("path", "")).startsWith("/v1/chat/completions")) continue;
            Map<String, Object> req = r.get("request") instanceof Map<?, ?> m ? (Map<String, Object>) m : Map.of();
            List<Map<String, Object>> msgs = (List<Map<String, Object>>) req.getOrDefault("messages", List.of());
            List<String> tail = new ArrayList<>();
            for (int i = msgs.size() - 1; i >= 0; i--)
                if ("tool".equals(msgs.get(i).get("role"))) tail.add(String.valueOf(msgs.get(i).get("content")));
                else break;
            String k = String.valueOf(r.getOrDefault("task", ""));
    ...
    public static String transcript(List<Map<String, Object>> recs, List<Turn> turns, Map<String, String[]> taskWindows) {
        Map<Object, List<String>> results = toolResults(recs);
        List<String> lines = new ArrayList<>(List.of("# Trajectory transcript (condensed by the harness)", ""));
        ...
        for (int i = 0; i < turns.size(); i++) {
            ...
            for (ToolCall c : t.toolCalls()) {
                String arg;
                try {
                    JsonNode d = JSON.readTree(c.args());
    ...
    public static Map.Entry<Integer, Map<String, Integer>> objectiveIndex(Map<String, Object> s, Map<String, Object> manifest) {
        Map<String, Integer> pen = new LinkedHashMap<>();
        ...
        List<Map<String, Object>> tasks = manifest.get("tasks") instanceof List<?> l ? (List<Map<String, Object>>) l : List.of();
        ...
        int total = pen.values().stream().mapToInt(Integer::intValue).sum();
    ...
    static boolean conclusiveRed(Object v) {
        if (!(v instanceof Map<?, ?> m)) return false;
        boolean ran = Boolean.TRUE.equals(m.get("ran"));
        boolean green = Boolean.FALSE.equals(m.get("green"));
        int executed = m.get("executed") instanceof Number n ? n.intValue() : 0;
        List<Integer> rcs = m.get("rc") instanceof List<?> l ? (List<Integer>) l : List.of(1);

#!/bin/zsh
# ace-service: Docker Desktop ON DEMAND. Installed as `docker` first on the agent's PATH. The first call starts Docker
...
log=${ACE_DOCKER_LOG:-/dev/null}; me=${0:A:h}
if [[ "${ACE_DOCKER_SHIM:-}" == "1" ]]; then echo "ace-service docker shim: recursion (no real docker CLI found)" >&2; exit 127; fi   # never call ourselves - exact sentinel: an unrelated ACE_DOCKER_SHIM=0/other value must not disable the shim
export ACE_DOCKER_SHIM=1
real=""
for d in ${(s.:.)PATH}; do
  ...
  # /usr/local/bin/docker -> /opt/ace-service/docker_shim.sh, the directory check above does not skip it
...
if [[ -z "$real" ]]; then echo "ace-service docker shim: no docker CLI found on PATH" >&2; exit 127; fi
...
if ! "$real" info >/dev/null 2>&1; then
  ...
  lock=${ACE_DOCKER_SHIM_LOCK:-/tmp/ace-service-docker-shim.lock}
  ...
  if "$real" info >/dev/null 2>&1; then
    ...
    echo "ace-service docker shim: Docker Desktop did not become ready within $((EPOCHSECONDS - t0))s (no install? out of memory?)" >&2
    exit 1
  fi
fi
exec "$real" "$@"

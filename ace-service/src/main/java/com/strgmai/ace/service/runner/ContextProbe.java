...
public final class ContextProbe {
    ...
    Post post(String endpoint, String key, Map<String, Object> body, int timeoutSec) {
        ...
        try {
            ...
            final double lat = elapsed(t0);
            final ObjectNode nu = usage == null || !usage.isObject() ? JSON.createObjectNode() : (ObjectNode) usage.deepCopy();
    ...
    public Map<String, JsonNode> models(final String endpoint, final String key) {
        try {
            ...
            final JsonNode data = JSON.readTree(r.body()).path("data");
    ...
    Map<String, Object> guardFacts(String since) {
        final java.util.regex.Pattern re = java.util.regex.Pattern.compile("usage ([\\d.]+)GB vs sizing target ([\\d.]+)GB, dynamic ceiling ([\\d.]+)");
        final List<Double> usage = new ArrayList<>(), target = new ArrayList<>(), ceiling = new ArrayList<>();
        final List<Path> files = new ArrayList<>(List.of(LOG));
    ...
    public static int positionalLimit(final String model, List<String> modelDirs, final int dflt) {
        for (String d : modelDirs) {
            final var cp = Path.of(d, model, "config.json");
    ...
    public static Object setModelCap(final String model, final int value) throws IOException {
        final ObjectNode d = Files.exists(MODEL_SETTINGS) ? (ObjectNode) JSON.readTree(Files.readString(MODEL_SETTINGS)) : JSON.createObjectNode();
        if (!d.has("models") || !d.get("models").isObject()) d.putObject("models");
        final ObjectNode entry = d.with("models").with(model);
        final Object prev = entry.has("max_context_window") ? entry.get("max_context_window").asInt() : null;
    ...
    public void restartServer(final String endpoint, final String key) {
        ...
        final List<Long> now = pids();
        final List<Long> survivors = old.stream().filter(now::contains).toList();
        try {
    ...
    static List<Long> pids() {
        final DockerService.Sh out = DockerService.sh(8, "pgrep", "-x", "omlx-server");
    ...
    Integer serverCap(String endpoint, String key, String model) {
        final JsonNode m = models(endpoint, key).get(model);
    ...
    public Map<String, Object> prepareCap(final String model, final String endpoint, final String key, List<String> modelDirs, Map<String, Object> o) throws IOException {
        final Integer before = serverCap(endpoint, key, model);
        final int wanted = positionalLimit(model, modelDirs, (Integer) o.get("positional_limit_default"));
        final Map<String, Object> info = new LinkedHashMap<>(Map.of("cap_before", before, "positional_limit", wanted, "cap_probe", before, "restarts", 0));
        ...
        final Integer now = serverCap(endpoint, key, model);
    ...
    public Object finalizeCap(final String model, final String endpoint, final String key, int usable, Map<String, Object> o) throws IOException {
        if (!Boolean.TRUE.equals(o.get("set_cap_to_measured")) || !Files.exists(OMLX_BIN)) return null;
        final Integer cur = serverCap(endpoint, key, model);
    ...
    public Setup setupKey(final String model, final String endpoint, final String key, List<String> modelDirs) throws Exception {
        ...
        if (Files.exists(MODEL_SETTINGS)) {
            final JsonNode ms = JSON.readTree(Files.readString(MODEL_SETTINGS)).path("models").path(model);
            final Map<String, Object> keep = new LinkedHashMap<>();
        ...
        facts.put("omlx_version", Files.exists(OMLX_BIN) ? com.strgmai.ace.service.docker.DockerService.sh(20, OMLX_BIN.toString(), "version").out().strip() : null);
        facts.put("host_mem", com.strgmai.ace.service.docker.DockerService.sh(10, "sysctl", "-n", "hw.memsize").out().strip());
        facts.put("docker_up", com.strgmai.ace.service.docker.DockerService.dockerRunning());
    ...
    private Step ask(final String endpoint, final String key, final String model, Map<String, Object> o, final int nLines, final int maxTokens, final boolean generate) {
        ...
        final JsonNode u = r.usage() == null ? JSON.createObjectNode() : r.usage();
    ...
    Step askRetry(String endpoint, String key, String model, Map<String, Object> o, int nLines, int maxTokens, boolean generate) {
        final Step r = ask(endpoint, key, model, o, nLines, maxTokens, generate);
    ...
    List<Step> decodeSamples(String endpoint, String key, String model, Map<String, Object> o, double tpl, double overhead) {
        final List<Step> samples = new ArrayList<>();
        for (List<Object> s : (List<List<Object>>) o.get("decode_samples")) {
            final int nTok = ((Number) s.get(0)).intValue(), outTok = ((Number) s.get(1)).intValue();
            final Step r = askRetry(endpoint, key, model, o, linesFor(nTok, tpl, overhead), outTok, true);
            samples.add(r);
        }
        return samples;
    }
    ...
    public Map<String, Object> probe(final String model, final String endpoint, final String key, Map<String, Object> o) {
        final var tStart = Instant.now().toString();
        final Integer cap = serverCap(endpoint, key, model);
        // calibrate tokens per line (two points), verified on every later step from the server's own prompt_tokens
        final Step a = ask(endpoint, key, model, o, 10, 8, false), b = ask(endpoint, key, model, o, 110, 8, false);
        ...
        tpl /= 100.0;
        final double overhead = (a.promptTokens() instanceof Number an ? an.doubleValue() : 0) - 10 * tpl;
        final List<Step> samples = decodeSamples(endpoint, key, model, o, tpl, overhead);
        ...
        final int top = hardMax - (Integer) o.get("probe_max_tokens") - 8;
        ...
        targets.add(top);
        final List<Step> curve = new ArrayList<>();
        int lastOk = 0, firstFail = 0;
        for (int target : targets) {
            final Step r = askRetry(endpoint, key, model, o, linesFor(target, tpl, overhead), (Integer) o.get("probe_max_tokens"), false);
            curve.add(r);
        ...
        if (firstFail > 0 && firstFail - lastOk > (Integer) o.get("refine_tokens")) {   // binary refinement between the last success and the failure
            ...
            for (int i = 0; i < 4 && hi - lo > (Integer) o.get("refine_tokens"); i++) {
                ...
                final Step r = askRetry(endpoint, key, model, o, linesFor(mid, tpl, overhead), (Integer) o.get("probe_max_tokens"), false);
                curve.add(r);
                if ("200".equals(r.status())) { lo = r.promptTokens() instanceof Number n ? n.intValue() : lo; lastOk = Math.max(lastOk, lo); }
                else hi = mid;
            }
        }
        final Step fail = curve.stream().filter(s -> !"200".equals(s.status())).findFirst().orElse(null);
        String binding = null;
        if (fail != null) {
            // the memory guard ALSO answers 400 "Prompt too long": a rejection well below the cap is memory, not the setting
            final boolean atCap = cap != null && firstFail >= cap - (Integer) o.get("step_tokens") - (Integer) o.get("probe_max_tokens");
        ...
        final Double shortMedian = shortDps.isEmpty() ? null : median(shortDps);
    ...
    public Map<String, Object> refreshDecode(final String model, final String endpoint, final String key, Map<String, Object> rec, Map<String, Object> o) {
        final double tpl = rec.get("tokens_per_line") instanceof Number n ? n.doubleValue() : 31.0;
    ...
    public Map<String, Object> parallelCapacity(final String model, final String endpoint, final String key, Map<String, Object> o, Double floor, final double tpl) throws Exception {
        final int overhead = 35;
        final var shared = new StringBuilder();
        for (int i = 1; i <= Math.max(1, (int) ((8000 - overhead) / tpl)); i++) shared.append(String.format(java.util.Locale.ROOT, LINE, i));
        final int tailLines = Math.max(1, (int) ((((Integer) o.get("parallel_prompt_tokens")) - 8000) / tpl));
        final int nonce = (int) (System.currentTimeMillis() / 1000) % 100000;
        ...
        for (int K = 1; K <= (Integer) o.get("max_parallel_probe"); K++) {
            final Post[] res = new Post[K];
            final long t0 = System.nanoTime();
            final Thread[] ths = new Thread[K];
            for (int k = 0; k < K; k++) {
                final int idx = k;
                ths[k] = new Thread(() -> {
            ...
            final List<Post> ok = Arrays.stream(res).filter(r -> r != null && r.status() == 200 && r.usage() != null && r.usage().hasNonNull("completion_tokens")).toList();
            if (K == 1 && !ok.isEmpty()) wall1 = wall;
            final Double speedup = wall1 > 0 ? Math.round(K * wall1 / wall * 100) / 100.0 : null;
            final List<Double> per = ok.stream().map(r -> r.usage().path("generation_tokens_per_second").asDouble(0)).filter(d -> d > 0).sorted().toList();
            final Double perMed = per.isEmpty() ? null : per.get(per.size() / 2);
            ...
            final boolean gaining = speedup != null && speedup >= 1 + ((Number) o.get("parallel_gain_per_agent")).doubleValue() * (K - 1);
            final boolean fastEnough = perMed != null && (floor == null || perMed >= floor);
    ...
    public static Double decodeAt(final List<double[]> curve, final double n) {
        if (curve == null || curve.isEmpty()) return null;
        final List<double[]> pts = new ArrayList<>(curve);
        ...
        for (int i = 1; i < pts.size(); i++)
            if (pts.get(i - 1)[0] <= n && n <= pts.get(i)[0]) {
                final double x0 = pts.get(i - 1)[0], y0 = pts.get(i - 1)[1], x1 = pts.get(i)[0], y1 = pts.get(i)[1];
    ...
    public static Map<String, Object> derive(Map<String, Object> rec, final BenchProperties props, final Integer taskWallSec) {
        final Map<String, Object> o = DEFAULTS;
        final int maxOk = rec.get("max_ok_prompt_tokens") instanceof Number n ? n.intValue() : 0;
        int memoryWindow;
        ...
        final Double dps = rec.get("decode_tps_short") instanceof Number n ? n.doubleValue() : null;
        final Double floor = dps == null ? null : Math.round(dps * ((Number) o.get("decode_floor_fraction")).doubleValue() * 10) / 10.0;
        final Double perfFloor = dps == null ? null : Math.round(dps * ((Number) o.get("perf_fraction")).doubleValue() * 10) / 10.0;
        final Integer perf = performanceWindow(rec, perfFloor);
        ...
        final Map.Entry<String, Integer> limited = candidates.entrySet().stream().min(Map.Entry.comparingByValue()).orElse(null);
        ...
        final int usable = Math.max(limited == null ? 2048 : limited.getValue(), 2048);
        final double scale = Math.max(((Number) o.get("pack_scale_min")).doubleValue(), Math.min(1.0, usable / (double) (int) o.get("pack_reference_window")));
        final int maxOut = Math.min(props.maxOutputTokens() == null ? 8192 : props.maxOutputTokens(), (int) (usable * ((Number) o.get("max_output_fraction")).doubleValue()) / 512 * 512);
        final int trigger = (int) (usable * ((Number) o.get("trigger_fraction")).doubleValue()) / 256 * 256;
        Integer genCap = null;
    ...
    public Map<String, Object> ensure(final BenchProperties props, final boolean fresh, final Integer taskWallSec, final Map<String, Object> overrides) throws Exception {
        final Map<String, Object> o = new LinkedHashMap<>(DEFAULTS);
        if (overrides != null) o.putAll(overrides);
        List<String> modelDirs = List.of();
        if (Files.exists(OMLX_SETTINGS)) {
            final JsonNode md = JSON.readTree(Files.readString(OMLX_SETTINGS)).path("model").path("model_dirs");
            if (md.isArray()) { List<String> dirs = new ArrayList<>(); md.forEach(n -> dirs.add(n.asText())); modelDirs = dirs; }
        }
        final Setup setup = setupKey(props.model(), props.endpoint(), props.apiKey() == null ? "" : props.apiKey(), modelDirs);
        Files.createDirectories(CACHE_DIR);
        final Path path = CACHE_DIR.resolve(props.model() + "-" + setup.key() + ".json");
        Map<String, Object> rec = null;
        if (!fresh && Files.exists(path)) {
            try {
                rec = JSON.readValue(Files.readString(path), Map.class);
                final double ageDays = (System.currentTimeMillis() - Files.getLastModifiedTime(path).toMillis()) / 86400_000.0;
                if (ageDays > (int) o.get("cache_max_age_days") || !Boolean.TRUE.equals(rec.get("ok"))) rec = null;
                    ...
                    final Double fl = rec.get("decode_tps_short") instanceof Number n ? Math.round(n.doubleValue() * ((Number) o.get("decode_floor_fraction")).doubleValue() * 10) / 10.0 : null;
        ...
        if (rec == null) {
            ...
            final Map<String, Object> capInfo = prepareCap(props.model(), props.endpoint(), props.apiKey() == null ? "" : props.apiKey(), modelDirs, o);
            ...
            if (Boolean.TRUE.equals(rec.get("ok")) && !"probe_limit".equals(rec.get("binding"))) {
                final Map<String, Object> d0 = derive(rec, props, taskWallSec);
    ...
    static String sha256(final Path p) throws Exception {

...
class ValidityTest {
    ...
    private static Map<String, Object> phase(final String id, Object rc, final String finish, final boolean artifact) {
    ...
    void aCrashedPhaseInvalidatesTheRun() {
        final var manifest = Map.of("mode", "monolithic", "phases", List.of(phase("p1_plan", 2, "stop", true)));
    ...
    void aLengthDeathWithoutAnArtifactIsAStopPolicyArtefact() {
        final var manifest = Map.of("mode", "monolithic", "phases", List.of(phase("p2_implementation", 0, "length", false)));
        final Map<String, Object> v = Validity.validity(manifest, facts(10, 0), 0.2);
    ...
    void aBudgetTimeoutIsAResultNotAnInvalidation() {
        final var manifest = Map.of("mode", "monolithic", "phases", List.of(phase("p2_implementation", 124, "length", true)));
        final Map<String, Object> v = Validity.validity(manifest, facts(10, 0), 0.2);
    ...
    void aTokenBudgetRefusalBeforeACrashDoesNotInvalidate() {
        ...
        final Map<String, Object> ph = new java.util.LinkedHashMap<>(manifest);
        final List<Map<String, Object>> phases = new java.util.ArrayList<>();
        final Map<String, Object> p = phase("p2_implementation", 3, "stop", true);
        p.put("token_budget_exhausted", true);   // the 429 refusal: over budget, not crashed
        phases.add(p);
        ph.put("phases", phases);
        final Map<String, Object> v = Validity.validity(ph, facts(10, 0), 0.2);
    ...
    void noRequestsOrHighErrorRateInvalidates() {
        ...
        final List<String> reasons = (List<String>) Validity.validity(manifest, facts(10, 3), 0.2).get("reasons");
